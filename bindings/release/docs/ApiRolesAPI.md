...
> PaginatedGalaxyRoleResponseList ApiV1RolesGet(ctx, ansibleRoleHref).XTaskDiagnostics(xTaskDiagnostics).Limit(limit).Offset(offset).Fields(fields).ExcludeFields(excludeFields).Execute()
...
func main() {
	...
	resp, r, err := apiClient.ApiRolesAPI.ApiV1RolesGet(context.Background(), ansibleRoleHref).XTaskDiagnostics(xTaskDiagnostics).Limit(limit).Offset(offset).Fields(fields).ExcludeFields(excludeFields).Execute()
	if err != nil {

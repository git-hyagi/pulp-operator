...
> SummaryResponse PypiRead(ctx, path).XTaskDiagnostics(xTaskDiagnostics).Fields(fields).ExcludeFields(excludeFields).Execute()
...
func main() {
	...
	resp, r, err := apiClient.PypiAPI.PypiRead(context.Background(), path).XTaskDiagnostics(xTaskDiagnostics).Fields(fields).ExcludeFields(excludeFields).Execute()
	if err != nil {

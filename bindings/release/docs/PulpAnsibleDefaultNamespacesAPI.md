# \PulpAnsibleDefaultNamespacesAPI

All URIs are relative to *http://pulp-web-svc-pulp.apps-crc.testing*

Method | HTTP request | Description
------------- | ------------- | -------------
[**PulpAnsibleGalaxyDefaultNamespacesList**](PulpAnsibleDefaultNamespacesAPI.md#PulpAnsibleGalaxyDefaultNamespacesList) | **Get** /pulp_ansible/galaxy/default/api/v3/namespaces/ | 
[**PulpAnsibleGalaxyDefaultNamespacesRead**](PulpAnsibleDefaultNamespacesAPI.md#PulpAnsibleGalaxyDefaultNamespacesRead) | **Get** /pulp_ansible/galaxy/default/api/v3/namespaces/{name}/ | 



## PulpAnsibleGalaxyDefaultNamespacesList

> PaginatedansibleAnsibleNamespaceMetadataResponseList PulpAnsibleGalaxyDefaultNamespacesList(ctx).XTaskDiagnostics(xTaskDiagnostics).Company(company).CompanyContains(companyContains).CompanyIcontains(companyIcontains).CompanyIexact(companyIexact).CompanyIn(companyIn).CompanyIregex(companyIregex).CompanyIstartswith(companyIstartswith).CompanyRegex(companyRegex).CompanyStartswith(companyStartswith).Limit(limit).MetadataSha256(metadataSha256).MetadataSha256In(metadataSha256In).Name(name).NameContains(nameContains).NameIcontains(nameIcontains).NameIexact(nameIexact).NameIn(nameIn).NameIregex(nameIregex).NameIstartswith(nameIstartswith).NameRegex(nameRegex).NameStartswith(nameStartswith).Offset(offset).Ordering(ordering).PrnIn(prnIn).PulpHrefIn(pulpHrefIn).PulpIdIn(pulpIdIn).Q(q).Fields(fields).ExcludeFields(excludeFields).Execute()
...
func main() {
	...
	resp, r, err := apiClient.PulpAnsibleDefaultNamespacesAPI.PulpAnsibleGalaxyDefaultNamespacesList(context.Background()).XTaskDiagnostics(xTaskDiagnostics).Company(company).CompanyContains(companyContains).CompanyIcontains(companyIcontains).CompanyIexact(companyIexact).CompanyIn(companyIn).CompanyIregex(companyIregex).CompanyIstartswith(companyIstartswith).CompanyRegex(companyRegex).CompanyStartswith(companyStartswith).Limit(limit).MetadataSha256(metadataSha256).MetadataSha256In(metadataSha256In).Name(name).NameContains(nameContains).NameIcontains(nameIcontains).NameIexact(nameIexact).NameIn(nameIn).NameIregex(nameIregex).NameIstartswith(nameIstartswith).NameRegex(nameRegex).NameStartswith(nameStartswith).Offset(offset).Ordering(ordering).PrnIn(prnIn).PulpHrefIn(pulpHrefIn).PulpIdIn(pulpIdIn).Q(q).Fields(fields).ExcludeFields(excludeFields).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PulpAnsibleDefaultNamespacesAPI.PulpAnsibleGalaxyDefaultNamespacesList``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `PulpAnsibleGalaxyDefaultNamespacesList`: PaginatedansibleAnsibleNamespaceMetadataResponseList
	fmt.Fprintf(os.Stdout, "Response from `PulpAnsibleDefaultNamespacesAPI.PulpAnsibleGalaxyDefaultNamespacesList`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPulpAnsibleGalaxyDefaultNamespacesListRequest struct via the builder pattern
...
## PulpAnsibleGalaxyDefaultNamespacesRead

> AnsibleAnsibleNamespaceMetadataResponse PulpAnsibleGalaxyDefaultNamespacesRead(ctx, name).XTaskDiagnostics(xTaskDiagnostics).Fields(fields).ExcludeFields(excludeFields).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.PulpAnsibleDefaultNamespacesAPI.PulpAnsibleGalaxyDefaultNamespacesRead(context.Background(), name).XTaskDiagnostics(xTaskDiagnostics).Fields(fields).ExcludeFields(excludeFields).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PulpAnsibleDefaultNamespacesAPI.PulpAnsibleGalaxyDefaultNamespacesRead``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `PulpAnsibleGalaxyDefaultNamespacesRead`: AnsibleAnsibleNamespaceMetadataResponse
	fmt.Fprintf(os.Stdout, "Response from `PulpAnsibleDefaultNamespacesAPI.PulpAnsibleGalaxyDefaultNamespacesRead`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPulpAnsibleGalaxyDefaultNamespacesReadRequest struct via the builder pattern

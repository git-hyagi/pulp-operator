...
------------- | ------------- | -------------
[**SigningServicesList**](SigningServicesAPI.md#SigningServicesList) | **Get** /api/v3/signing-services/ | List signing services
...
> PaginatedSigningServiceResponseList SigningServicesList(ctx).XTaskDiagnostics(xTaskDiagnostics).Limit(limit).Name(name).Offset(offset).Ordering(ordering).PrnIn(prnIn).PulpHrefIn(pulpHrefIn).PulpIdIn(pulpIdIn).Q(q).Fields(fields).ExcludeFields(excludeFields).Execute()
...
func main() {
	...
	resp, r, err := apiClient.SigningServicesAPI.SigningServicesList(context.Background()).XTaskDiagnostics(xTaskDiagnostics).Limit(limit).Name(name).Offset(offset).Ordering(ordering).PrnIn(prnIn).PulpHrefIn(pulpHrefIn).PulpIdIn(pulpIdIn).Q(q).Fields(fields).ExcludeFields(excludeFields).Execute()
	if err != nil {

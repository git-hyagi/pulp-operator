...
------------- | ------------- | -------------
[**ImportersCorePulpCreate**](ImportersPulpAPI.md#ImportersCorePulpCreate) | **Post** /api/v3/importers/core/pulp/ | Create a pulp importer
[**ImportersCorePulpDelete**](ImportersPulpAPI.md#ImportersCorePulpDelete) | **Delete** /{pulp_importer_href} | Delete a pulp importer
[**ImportersCorePulpList**](ImportersPulpAPI.md#ImportersCorePulpList) | **Get** /api/v3/importers/core/pulp/ | List pulp importers
...
> PulpImporterResponse ImportersCorePulpCreate(ctx).PulpImporter(pulpImporter).XTaskDiagnostics(xTaskDiagnostics).Execute()
...
func main() {
	...
	resp, r, err := apiClient.ImportersPulpAPI.ImportersCorePulpCreate(context.Background()).PulpImporter(pulpImporter).XTaskDiagnostics(xTaskDiagnostics).Execute()
	if err != nil {
...
> PaginatedPulpImporterResponseList ImportersCorePulpList(ctx).XTaskDiagnostics(xTaskDiagnostics).Limit(limit).Name(name).NameContains(nameContains).NameIcontains(nameIcontains).NameIexact(nameIexact).NameIn(nameIn).NameIregex(nameIregex).NameIstartswith(nameIstartswith).NameRegex(nameRegex).NameStartswith(nameStartswith).Offset(offset).Ordering(ordering).PrnIn(prnIn).PulpHrefIn(pulpHrefIn).PulpIdIn(pulpIdIn).Q(q).Fields(fields).ExcludeFields(excludeFields).Execute()

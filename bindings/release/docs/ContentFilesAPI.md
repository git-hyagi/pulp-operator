...
------------- | ------------- | -------------
[**ContentFileFilesCreate**](ContentFilesAPI.md#ContentFileFilesCreate) | **Post** /api/v3/content/file/files/ | Create a file content
[**ContentFileFilesList**](ContentFilesAPI.md#ContentFileFilesList) | **Get** /api/v3/content/file/files/ | List file contents
...
> AsyncOperationResponse ContentFileFilesCreate(ctx).RelativePath(relativePath).XTaskDiagnostics(xTaskDiagnostics).Repository(repository).PulpLabels(pulpLabels).Artifact(artifact).File(file).Upload(upload).FileUrl(fileUrl).Execute()
...
func main() {
	...
	resp, r, err := apiClient.ContentFilesAPI.ContentFileFilesCreate(context.Background()).RelativePath(relativePath).XTaskDiagnostics(xTaskDiagnostics).Repository(repository).PulpLabels(pulpLabels).Artifact(artifact).File(file).Upload(upload).FileUrl(fileUrl).Execute()
	if err != nil {

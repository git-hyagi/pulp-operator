	...
	for plugin, resource := range plugins {
		...
		go func(pluginName string, resource map[string]string) {
			...
			for resourceType, endpoint := range resource {
				...
				if err != nil {
					log.Error(err, "############ Failed to create file distribution ###################")
				}
...
func deletePulpResource(pulpResource *pulpv1.PulpResource, plugin, resource, endpoint string, pulpClient pulpClient) {
	log := pulpClient.Log
...
func updatePulpResource(pulpResource *pulpv1.PulpResource, plugin, resource string, pulpClient pulpClient, endpoint string) {
	log := pulpClient.Log
	...
	if err != nil {
		pulpClient.Log.Error(err, "Failed to make a GET request to "+endpoint)
	}

	if response.Count == 0 {
		pulpClient.Log.Error(err, "Failed to find the "+plugin+" "+resource+" "+resourceNameToBeUpdated)
		return
	}
	pulp_href := response.Results[0].(map[string]interface{})["pulp_href"].(string)
	log.V(1).Info("GET response " + pulp_href)
	...
	if statusResourceField.Elem().FieldByName("Name").String() != specResourceField.Elem().FieldByName("Name").String() {
		...
		if err != nil {
			pulpClient.Log.Error(err, "Failed to make a PATCH request to "+pulp_href)
		}
		log.Info(plugin + " " + resource + " " + resourceNameToBeUpdated + " updated!")
	}
}

func createSyncResource(pulpResource *pulpv1.PulpResource, plugin string, pulpClient pulpClient) {
	...
	if !field_empty(statusPluginField) {
		if reflect.DeepEqual(specRemoteField.Interface(), statusPluginField.Elem().FieldByName("Remote").Interface()) &&
			...
			log.Info("Already synced " + plugin + " plugin before, nothing to do")
			return
		}
	}
	...
	repositoryHref := get_pulp_href(log, plugin, "Repository", specRepositoryName, pulpClient)
	if repositoryHref == "" {
		return
	}

	remoteHref := get_pulp_href(log, plugin, "Remote", specRemoteName, pulpClient)
	if remoteHref == "" {
		return
	}

	distributionHref := get_pulp_href(log, plugin, "Distribution", specDistributionName, pulpClient)
	if distributionHref == "" {
		return
	}
	...
	sync := pulpv1.PulpSync{Remote: remoteHref, Mirror: false, SignedOnly: false}
...
func (r *RepoManagerPulpResourceReconciler) syncPulpResources(pulpResource *pulpv1.PulpResource, pulpClient pulpClient) {
	var wg sync.WaitGroup
...
func get_pulp_href(log logr.Logger, plugin, resourceType, resourceName string, pulpClient pulpClient) string {

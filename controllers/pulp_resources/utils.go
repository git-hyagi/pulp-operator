...
func make_request(method, addr, user, pass string, body any) (responseBody, error) {
	...
	// to-do rename this var cammel case
	response_body, err := io.ReadAll(resp.Body)
	response := &responseBody{}
	json.Unmarshal(response_body, response)
	if err != nil {
		return responseBody{}, err
	}

	if resp.StatusCode/100 != 2 {
		return responseBody{}, fmt.Errorf(string(response_body))
	}
...
func create_status_fields(pulpResource *pulpv1.PulpResource, plugin string, resource string) {
